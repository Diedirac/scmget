...
class AIPlayer: public ChessPlayer {
	...
	public:
		...
		*/
		...
		/*
		* Ask player what to do next and record
		*/
		bool getMove(ChessBoard & board, Move & move, unsigned long long* nodeCount) const;
		...
		*/ 
		...
		/*
		* MinMax search for best possible outcome with node counter
		*/ 
		int evalAlphaBeta(ChessBoard & board, int color, int depth, int alpha, int beta, bool quiescent, unsigned long long* nodeCount) const;
